...
def top_20_keyword():
	...
	for word in keyword_order:
		if len(top_20_list) < 20:
			...
			for word_in_top20 in top_20_list:
				# if word_in_top20 count is lesser than the current min count then update min
				if keyword_dict[word_in_top20] < keyword_dict[min_top20]:
					min_top20 = word_in_top20

			# compare minimum count keyword in top 20 list and the current word in all search
			# remove minimum count keyword in top 20 list if current word is greater
